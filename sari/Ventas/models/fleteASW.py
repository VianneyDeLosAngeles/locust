...
class Flete(models.Model):
    ...
    def get_fromfile(self, excel_path):
        """Funcion para extrar la informacion del envio de e-commerce  e insertar la info
        ...
        for i in range(df.shape[i]-1):
            try: 
                ...
                id_flete=self.__class__.objects.create(
                        ...
                        tipo = df.iloc[i,10],\
                        fechaFactura = pd.to_datetime(df.iloc[i,11]),\
